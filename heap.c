...
// https://github.com/tsoding/memalloc/blob/master/heap.c
    ...
    if (size_words > 0) {
        ...
        for (size_t i = 0; i < freed_chunks.count; i++) {
            ...
            if (chunk.size >= size_words) {
                ...
                chunk_list_insert(&allocated_chunks, chunk.start, size_words);
...
void heap_free(void *ptr) {
    if (ptr != NULL) {
        ...
        chunk_list_insert(&freed_chunks, allocated_chunks.chunks[index].start, allocated_chunks.chunks[index].size);
...
static void mark_region(const uintptr_t *start, const uintptr_t *end) {
    for (; start < end; start++) {
        ...
        for (size_t i = 0; i < allocated_chunks.count; i++) {
            Chunk chunk = allocated_chunks.chunks[i];
